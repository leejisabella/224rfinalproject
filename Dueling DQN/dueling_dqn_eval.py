...
STATE_DIM = 728
NUM_GAMES = 100
MODEL_PATH = "Dueling DQN/results/dueling_dqn_model.pt"

def evaluate_dueling_dqn(agent, num_games=NUM_GAMES, log_file="Dueling DQN/results/dueling_dqn_eval_results.txt"):
    total_p_points = 0
    total_b_points = 0
    total_p_wins = 0
    total_b_wins = 0
    ...
    with open(log_file, "w") as f:
        for game_idx in tqdm(range(num_games), desc="Evaluating Dueling DQN"):
            ...
            f.write(f"Game {game_idx+1} | Dueling DQN: {player_points}, Bot: {bot_points}\n")

        f.write("\n--- Dueling DQN Evaluation Summary ---\n")
        f.write(f"Total Dueling DQN Points: {total_p_points}\n")
        f.write(f"Total Bot Points: {total_b_points}\n")
        f.write(f"Average Dueling DQN Points: {total_p_points / num_games:.2f}\n")
        f.write(f"Average Bot Points: {total_b_points / num_games:.2f}\n")
        f.write(f"Dueling DQN Wins: {total_p_wins}/{num_games} ({100 * total_p_wins / num_games:.1f}%)\n")
